(() => {
  const summarySlide = document.getElementById("slide-30");
  if (!summarySlide) return;

  document.getElementById("slide-ray-approximation")?.remove();

  const slide = document.createElement("section");
  slide.id = "slide-ray-approximation";
  slide.className = "slide content-slide ray-approx-slide";
  slide.innerHTML = String.raw`
  ...
  slide.append(
    document.createComment(
      "[Sources] Xu et al. (2026+), manuscript Section 2.2, equations (8)–(12)."
    )
  );

  summarySlide.before(slide);
